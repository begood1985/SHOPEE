...
def reconcile_sales_and_receipts(sales_df: pd.DataFrame, receipts_df: pd.DataFrame) -> pd.DataFrame:
    """Concilia vendas e recebimentos com base no cálculo Bruto - Taxas - Descontos."""
    ...
    # Identificar colunas para a fórmula
    ...
    # Cálculo da fórmula por linha (pedido)
    ...
    receipts_summary = receipts_df.groupby("_receipt_order_id").agg(
        ...
        arquivos_origem=("_receipt_source_file", lambda s: ", ".join(sorted(set(s.astype(str)))))
    ...
    def classify(row):
        ...
        if abs(diff) <= 0.01: return "Recebido integralmente"
